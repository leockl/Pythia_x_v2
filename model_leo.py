...
def lambda_handler(event, context):
    ...
    if "agent1" in sanitized_temporary_response:
        ...
        sql_code = retrieveAndGenerate(sql_agent_system_content + f"\nConvert this natural language question to SQL code: \
                                       {sanitized_query}", kb_id, model_arn, session_id)['output']['text']
        ...
        summary = retrieveAndGenerate(f"convert list of dictionaries table to natural language summary: {table_data}", \
                                      kb_id, model_arn, session_id)['output']['text']
        # question_and_answer_chat_history += f"\nAssistant: {summary}"
        ...
        # Get response from Recharts AI agent
        recharts_code = retrieveAndGenerate(f"Based on the provided table data and user query, generate only the Recharts code required \
                                            to visualize the data. Do not include any explanations, descriptions, or other unrelated text. \
                                            Only output the necessary Recharts code.\n\nTable data:\n{table_data}\n\nUser query: {query}", \
                                            kb_id, model_arn, session_id)['output']['text']
        ...
        # Get response from sql AI agent
        sql_code = retrieveAndGenerate(sql_agent_system_content + f"\nconvert natural language to sql code: {sanitized_query}", \
                                       kb_id, model_arn, session_id)['output']['text']
        ...
        # Get response from sql AI agent
        sql_code = retrieveAndGenerate(sql_agent_system_content + f"convert natural language to sql code: {sanitized_query}", \
                                       kb_id, model_arn, session_id)['output']['text']
        ...
        description = retrieveAndGenerate(f"convert list of dictionaries table to natural language description: {table_data}", \
                                          kb_id, model_arn, session_id)['output']['text']
